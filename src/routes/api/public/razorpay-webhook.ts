  ...
  server: {
    handlers: {
      POST: async ({ request }) => {
        ...
        if (name.startsWith("refund.") && event.payload?.refund?.entity?.payment_id) {
          await supabaseAdmin
            .from("subscriptions")
            .update({ status: "refunded", expires_at: new Date().toISOString(), updated_at: new Date().toISOString() })
